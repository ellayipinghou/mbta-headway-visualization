...
import { setStationStats, setDirectionData, COLORS } from './tooltips.js';
...
Promise.all([fetchDirections(), fetchDirectionsByLine()])
...
function getFilterState() {
    const selectedStations = Array.from(state.sel).filter(sid => {
        ...
        // Exclude stations whose lines are all inactive.
...
function fetchHeadwayData() {
    ...
    _fetchTimer = setTimeout(async () => {
        try {
            ...
            const avgHeadway = n
                ? entries.reduce((s, e) => s + e.avgHeadway, 0) / n
                : 0;

            const stddevHeadway = n
                ? entries.reduce((s, e) => s + e.stddevHeadway, 0) / n
                : 0;
...
document.querySelector('.glx-btns').addEventListener('click', e => {
    ...
    stations.forEach(s => {
        ...
        if (state.glx === 'glx' && !s.isGLX) {
            state.sel.delete(s.id);
        }
...
document.querySelectorAll('.dir-cb').forEach(cb => {
    cb.addEventListener('change', () => {
        if (cb.checked) {
            state.directions.add(cb.value);
        } else {
            state.directions.delete(cb.value);
        }
...
document.querySelectorAll('.lc').forEach(cb => {
    cb.addEventListener('change', () => {
        if (cb.checked) {
            ...
            stations.forEach(s => {
                if (s.lines.includes(cb.value)) {
                    state.sel.add(s.id);
                }
            });

            // Re-enable directions that are now available.
...
linesAllCb.addEventListener('change', () => {
    ...
    if (linesAllCb.checked) {
        ['North', 'South', 'East', 'West', 'Inbound', 'Outbound'].forEach(d =>
...
function renderStopList() {
    ...
    list.innerHTML = filtered.map(s => {
        ...
        const dots = s.lines.map(line => {
            ...
            let letter = line.startsWith('Green-')
                ? line.split('-')[1]
                : '';
            ...
            return `
                <span class="ld" style="background:${c}">${letter}</span>
            `;
        }).join('');
...
function syncDirsAll() {
    const enabledCbs = Array.from(document.querySelectorAll('.dir-cb'))
        .filter(c => !c.disabled);
...
function syncDirAvailability() {
    ...
    if (availableDirs.size === 0) {
        ...
        if (dirsLabel) {
            dirsLabel.style.opacity = '0.35';
        }
    } else {
        dirsAllCb.disabled = false;

        if (dirsLabel) {
            dirsLabel.style.opacity = '1';
        }
    }
...
function syncSelectAll() {
    ...
    if (item) {
        item.style.opacity = hasLines ? '1' : '0.35';
    }
}
