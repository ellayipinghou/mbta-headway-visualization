...
function smoothPath(pts, r) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
        ...
        const p1x = x1 - cr * (x1 - x0) / d1;
        const p1y = y1 - cr * (y1 - y0) / d1;
        const p2x = x1 + cr * (x2 - x1) / d2;
        const p2y = y1 + cr * (y2 - y1) / d2;

        const mx = (p1x + 2 * x1 + p2x) / 4;
        const my = (p1y + 2 * y1 + p2y) / 4;
...
function drawPath(d, color, layer, opts = {}) {
    ...
    if (opts.lines) attrs['data-lines'] = opts.lines;
    if (opts.glx !== undefined) attrs['data-glx'] = opts.glx;

    (layer || gLineLayer).appendChild(el('path', attrs));
}
...
function drawStations(arr, color, lineName) {
    for (const [name, x, y, ld, tm, tilt] of arr) {
        ...
        if (drawnElements.has(key)) {
            const elements = drawnElements.get(key);

            elements.forEach(element => {
                const existing = element.getAttribute('data-lines') || '';

                if (!existing.split(',').includes(lineName)) {
                    element.setAttribute('data-lines', existing + ',' + lineName);
                }
            });
        ...
        if (hitTarget) {
            ...
            hitTarget.addEventListener('click', e => {
                ...
                const isActive =
                    stationObj.lines.some(l => state.lines.has(l)) &&
                    (
                        state.glx === 'all' ||
                        (state.glx === 'glx' && stationObj.isGLX) ||
                        (state.glx === 'non-glx' && !stationObj.isGLX)
                    );

                if (!isActive) return;

                if (state.sel.has(stationObj.id)) {
                    state.sel.delete(stationObj.id);
                } else {
                    state.sel.add(stationObj.id);
                }
        ...
        if (tilt && ld === 'b') anchor = 'end';
        if (tilt && ld === 'a') anchor = 'start';

        const fontSize = tm ? 10.5 : 8.5;
        const fontWeight = tm ? '700' : '400';
        ...
        if (tilt) attrs.transform = `rotate(${tilt}, ${tx}, ${ty})`;
...
function drawMap() {
    const {
        redMain, redAsh, redBra, matt,
        oraN, oraHub, oraS, blu,
        gEN_upper, gEN_lower, gDU, gBCDE, gBCD,
        gHub, gES, gB, gC, gD
    } = mapData;

    // 1. Line paths.
    drawPoly(gEN_upper, C.grn, gLineLayer, {
        lines: 'Green-E',
        glx: 'true'
    });

    drawPoly(gEN_lower, C.grn, gLineLayer, {
        lines: 'Green-D,Green-E',
        glx: 'false'
    });

    drawPoly(gDU, C.grn, gLineLayer, {
        lines: 'Green-D',
        glx: 'false'
    });

    // Lechmere → Government Center via North Station/Haymarket.
    const gDEHub = [...gBCDE.slice(0, 2), ...gHub, gBCDE[2]];

    drawPoly(gDEHub, C.grn, gLineLayer, {
        lines: 'Green-D,Green-E'
    });

    // Government Center → Copley, all four branches.
    const gBCDECopley = gBCDE.slice(2);

    drawPoly(gBCDECopley, C.grn, gLineLayer, {
        lines: 'Green-B,Green-C,Green-D,Green-E'
    });

    drawPoly(gBCD, C.grn, gLineLayer, {
        lines: 'Green-B,Green-C,Green-D'
    });

    drawPoly(gES, C.grn, gLineLayer, {
        lines: 'Green-E',
        glx: 'false'
    });

    drawPoly(gB, C.grn, gLineLayer, {
        lines: 'Green-B'
    });

    drawPoly(gC, C.grn, gLineLayer, {
        lines: 'Green-C'
    });

    drawPoly(gD, C.grn, gLineLayer, {
        lines: 'Green-D'
    });
    ...
    drawPoly(matt, C.red, gLineLayer, {
        dash: '10 6',
        lines: 'Mattapan'
    });
    ...
    drawPoly([...oraNFull, ...oraS], C.ora, gLineLayer, {
        lines: 'Orange'
    });

    drawPoly(blu, C.blu, gLineLayer, {
        lines: 'Blue'
    });
    ...
    [
        ...
        gLabelLayer.appendChild(element);
    });
}
...
export function initializeMap({ update }) {
    ...
    gLineLayer = el('g', { id: 'lines' });
    gStationLayer = el('g', { id: 'stations' });
    gLabelLayer = el('g', { id: 'labels' });
    ...
    document.querySelectorAll('.line-label').forEach(labelEl => {
        labelEl.addEventListener('mouseenter', e => {
            showLineTooltip(
                e,
                labelEl.getAttribute('data-line-id')
            );
        });
