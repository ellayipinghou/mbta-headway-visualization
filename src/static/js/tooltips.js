...
function computeLineStats(lineId) {
    ...
    const avgHeadway =
        withStats.reduce(
            (sum, id) =>
                sum + stationStats[id].avgHeadway,
            0
        ) / withStats.length;
    ...
    const stddevHeadway =
        withStats.reduce(
            (sum, id) =>
                sum + stationStats[id].stddevHeadway,
            0
        ) / withStats.length;
...
export function showLineTooltip(e, lineId) {
    const color =
        COLORS[lineId] || '#00843D';


    const displayName =
        LINE_DISPLAY_NAMES[lineId] || lineId;
    ...
    const stats =
        computeLineStats(lineId);


    const lineDirs =
        getLineDirections(lineId);
    ...
    const partial =
        stats &&
        stats.activeCount < stats.totalCount / 2
            ? `
                <div class="tt-glx">
